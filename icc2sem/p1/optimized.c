...
unsigned int* better_search_and_count(int** matriz, int* vetor, int m, int z) {
    ...
    for (int k = 0; k < z; k++) {
		...
		for (start = 0; start < m; start++) {
			if (matriz[start][start] < element) break;
		}

        for (int i = start; i < m; i++) {
            if (element < matriz[i][i]) {
                // checa pra tras
                for (int j = i - 1; j >= 0; j--)
                    if (matriz[i][j] == element) con[k]++;

				// se for menor que essa diagonal, não vale a pena checar daqui pra frente
                break;

            } else if (element > matriz[i][i]) {
                // checa pra frente
                for (int j = i + 1; j < m; j++)
                    if (matriz[i][j] == element) con[k]++;
...
int main (int argc, char* argv[]) {
	...
	clock_t start = clock();
	unsigned int* c = better_search_and_count(mat, vet, m, z);
	clock_t end = clock();
